...
### The Challenge
A responsive, interactive hotel booking confirmation page designed for a boutique guesthouse in Cassis, France. This project features a branded sidebar alongside an interactive stack of overlapping cards—including an itemized stay receipt and a personalized welcome note from the host.

### Core Features
* **Interactive Card Stack:** Visual overlapping receipt and host note cards that fan out smoothly on desktop hover.
* **One-Click Wi-Fi Copy:** JavaScript clipboard integration allowing guests to quickly copy the guest network password.
* **Instant Print View:** Built-in print capability with clean CSS media query print optimization (hides dashboard UI and isolates the receipt).
* **Dynamic Calendar Event:** Generates and downloads a native `.ics` file so guests can directly save their booking dates to Apple, Google, or Outlook calendars.
* **Responsive Layout:** Adaptive design that collapses into a toggleable mobile navigation menu and stacked card view on smaller screens.

### Tech Stack
* **HTML5:** Semantic markup (`<aside>`, `<main>`, `<article>`, `<header>`).
* **CSS3:** Custom CSS Variables, Flexbox, Grid, CSS Transforms (`rotate`, `translate`), and `@media print` query rules.
* **JavaScript (ES6):** Async Clipboard API, dynamic `Blob` file generation, and DOM event manipulation.

---

## What I learned
<!-- Paste your "What I learned" code snippets here -->
